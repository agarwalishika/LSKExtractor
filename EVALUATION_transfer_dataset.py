...
def main(model_name, suffix):
    data_files = glob(f"generations_json/blend*{model_name}{suffix}.json")

    train_centers, clustered_training_points = get_clusters("blend")
    assert len(data_files) == 16

    train_langs = get_langs(clustered_training_points, data_files)
    with open(f'clustering_metadata/cluster_langs_lsk_B2CA_{NUM_CLUSTERS}|{model_name}.pkl', 'wb+') as f:
        pickle.dump(train_langs, f)

    with open(f'data/culture_atlas/data_English.pkl', 'rb') as f:
    ...
    data_files = glob(f"generations_json/culture_atlas*{model_name}{suffix}.json")
    datasets = {}
    for data_file in data_files:
    ...
    with open(f'results_transfer_dataset.txt', 'a+') as f:
        if "NR" in data_file:
            f.write(f"NR,{model_name}\tculture_atlas\t{accuracy}\n")
        else:
            f.write(f"WR,{model_name}\tculture_atlas\t{accuracy}\n")
